...
tokenizer, model = load__model()

def get__embeddings(texts, model, tokenizer):
    # Tokenize the input texts
    inputs = tokenizer(texts, padding=True, truncation=True, return_tensors="pt", max_length=512)

    # Move inputs to GPU (cuda:0) – this is crucial
    for key in inputs:
        inputs[key] = inputs[key] # Explicitly move to CUDA device
    ...
    embeddings = outputs.last_hidden_state[:, 0, :].cpu().numpy()  # move to CPU for numpy conversion
...
class Embeddings(Embeddings):
    def embed_documents(self, texts):
        embeddings = get__embeddings(texts, model, tokenizer)
        return np.array([upscale_embedding(emb) for emb in embeddings])
    
    def embed_query(self, text):
        embedding = get__embeddings([text], model, tokenizer)[0]
